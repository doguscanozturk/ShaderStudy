	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 frag(v2f i) : SV_Target
			{
                ...
                i.screenuv.y = 1- i.screenuv.y; // Screen y coordinates are reverse in DirectX 
				float4 ambient = tex2D(_AmbientTex, (i.screenuv + offset * _GlobalRefractionMag * 5) * 2);
				float4 worldRefl = tex2D(_GlobalRefractionTex, i.screenuv + offset.xy * _GlobalRefractionMag) * i.color;
				color.rgb = (color.rgb + ambient.rgb) * (1.0 - worldRefl.a * _GlobalVisibility)
				  	+ worldRefl.rgb * worldRefl.a * _GlobalVisibility;

                //i.screenuv.y = 1- i.screenuv.y; // Screen y coordinates are reverse in DirectX 
                //float4 color = tex2D(_MainTex, i.uv) * i.color;
                //float4 boringRefl = tex2D(_GlobalRefractionTex, i.screenuv) * i.color;
                //color.rgb = color.rgb * (1.0 - boringRefl.a) + boringRefl.rgb * boringRefl.a;
				return color;
			}
			ENDCG
		}
